...
export RUN_NAME=branson-tuolumne-singlenode-gpu-throughput-30-release
...
function build() {
  ...
  cmake -DCMAKE_BUILD_TYPE=Release -DUSE_HIP=ON -DUSE_GPU=ON -DN_GROUPS=30 -DCMAKE_INSTALL_PREFIX=$HOME/binaries/$RUN_NAME $HOME/branson/src/
